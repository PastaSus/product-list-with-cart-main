function AddToCart({ className, quantity, onAdd, onRemove }) {
  return quantity === 0 ? (
    <button
      className={`flex cursor-pointer items-center gap-2 rounded-full border-1 border-[var(--Rose-400)] bg-[var(--Rose-50)] px-5 py-3 font-medium md:min-w-[160px] ${className}`}
      onClick={onAdd}
    >
